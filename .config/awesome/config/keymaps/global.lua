...
local M = {
  awesome = {
    ...
    {{}, RETURN, function () awful.spawn(TERM) end, "open a terminal"},
  },
  ...
  client = {
    ...
    {{ CTRL }, "n", function () local c = awful.client.restore() if c then c:activate({ raise = true, context = "key.unminimize" }) end end, "restore minimized"},
